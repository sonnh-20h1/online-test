...
class UserController extends Controller
{
    ...
    public function GetUserAction($request, $response)
    {
        $result = $this->database->select('user_exam',
            [
                '[>]ol_account_google' => ['IDUSER' => 'id'],
                '[>]exam' => ['IDEXAM' => 'IDEXAM'],
                '[>]subjects' => ['exam.SUBID' => 'SUBID'],
            ],
            [
                'user_exam.ID_UX(id)',
                'user_exam.SCORE(score)',
                'user_exam.CONFIRM(submited)',
                'user_exam.DATEEXAM(date)',
                'user_exam.start_date',
                'ol_account_google.email',
                'ol_account_google.name',
                'ol_account_google.imageUrl(avatar)',
                'ol_account_google.accessToken(token)',
                'exam.EXAMTEXT(exam_name)',
                'exam.RANDOMEXAM(random)',
                'exam.status(exam_type)',
                'exam.IDEXAM(id_exam)',
                'subjects.SUBID(id_sub)',
                'subjects.SUBTEXT(subject_name)',
            ],
            [
                'exam.status[!]' => 3,
                'ORDER' => ['user_exam.start_date' => 'DESC'],
            ]
        );
